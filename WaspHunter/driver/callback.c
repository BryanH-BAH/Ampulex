...
// this is the windows kernel driver version of windows.h - it contains our windows types and other header info.
#include <Ntddk.h>
...
)
{
	...
	// if our ObjectType is a Process, then our PVOID will be a PEPROCESS struct, but that's a windows-internal "opaque" structure so only operate on that struct using win api functions.
    if (PreInfo->ObjectType == *PsProcessType)  {
		...
		// if the TdProtectedTargetProcessId (the originating process) is null, it's probably the kernel.  We can't fight other kernel drivers, just assume we don't need to log that.
		if (TdProtectedTargetProcessId != 0 && PreInfo->Object != PsGetCurrentProcess() && PreInfo->Operation == OB_OPERATION_HANDLE_CREATE) {
			UINT32 request_origin = (UINT32)(TdProtectedTargetProcessId);
			UINT32 target_pid = (UINT32)PsGetProcessId(PreInfo->Object);
			EventWriteCrossProcess(NULL, request_origin, target_pid, 0x0, 1);
		}
		...
		// remove this code since we actually want to check all processes.
		...
		// if the process requesting a new handle to a thread wasn't the process itself, thats, uh, bad, mkay
		// sometimes TdProtectedTargetProcessId is NULL i don't know why may have something to do with when you start the monitoring vs the program doing the opening.
		if (TdProtectedTargetProcessId != 0 && ProcessIdOfTargetThread != PsGetCurrentProcessId() && PreInfo->Operation == OB_OPERATION_HANDLE_CREATE) {
			DbgPrintEx(
				DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
				"ObCallbackTest: CBTdPreOperationCallback: ProcessID 0x%p was detected OpenThread() manipulating Process %d\n", (PVOID)TdProtectedTargetProcessId, ProcessIdOfTargetThread);

			UINT32 request_origin = (UINT32)(TdProtectedTargetProcessId);
			UINT32 target_pid = (UINT32)(ProcessIdOfTargetThread);

			// I don't know how to get the thread that was being targeted so for now i'll set it to 0x0
			// last arg defines it as an OpenThread event.
			EventWriteCrossProcess(NULL, request_origin, target_pid, 0x0, 2);
		}
    }
    else    {
        DbgPrintEx (
