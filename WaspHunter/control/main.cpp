...
void TcPrintUsage()
{
    puts ("Usage:");
    puts ("");
    puts("    ObCallbackTestCtrl.exe -install -uninstall -start -stop [-?]");
    puts("     -install        install driver");
    puts("     -uninstall      uninstall driver");
    puts("     -start          start recording cross-process events");
    puts("     -stop           stop recording cross-process events");
}
...
)
{
    ...
    if (argc > 1)
    {
        ...
        if (0 == wcscmp (arg, L"-stop")) {
            TcRemoveProtection();
        } else
        if (0 == wcscmp (arg, L"-start")) {
...
)
{
	...
	// we don't need a process name anymore. we watch all the things
    pwProcessName = L"";
